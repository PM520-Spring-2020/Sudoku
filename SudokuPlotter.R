...
# In order to use this version you need to define a 9x9 matrix called NumberOfErrors 
...
SquareChanger<-function(z,FixedElementIndicator,WhichSquare)
{
  ...
  # switch two of them
  HowMany<-length(NonFixedElements)
  Which<-sample(1:HowMany,2,replace=FALSE)
  Temp<-NonFixedElements[Which[1]]
  NonFixedElements[Which[1]]<-NonFixedElements[Which[2]]
  NonFixedElements[Which[2]]<-Temp
  cat("\nSwitching numbers",NonFixedElements[Which[1]]," and ",NonFixedElements[Which[2]]," in square ",WhichSquare)
  ...
  for (i in iIndex:(iIndex+2)){
    for (j in jIndex:(jIndex+2)){
      if (FixedElementIndicator[i,j]==0){
        ...
        z[i,j]<-NonFixedElements[1]
        NonFixedElements<-NonFixedElements[-1]
      }
    }
  }  
...
SwitchSomethingWithThis<-function(z,w,WhichSq,i,j)
{
  NonFixedElements<-vector()
  iIndex<-1+3*((WhichSquare-1)%%3)   # the i coordinate of the top-left entry in that square
  jIndex<-1+3*floor((WhichSquare-1)/3)    # the j coordinate of the top-left entry in that square
...
# a function that picks an element to switch in a square, but makes sure the first such element is causing an error
GridChanger<-function(z,w)
{
  # how many elements are causing errors
  HowMany<-length(NumberOfErrors[NumberOfErrors>0])
  # pick one
  Which<-sample(1:HowMany,1)
  # where is this element
  ErrorCount<-0
  i<-0
  while (ErrorCount<Which)
  {
    i<-i+1
    j<-0
    while ((ErrorCount<Which)&&(j<9)){
      j<-j+1
      if (NumberOfErrors[i,j]>0){ErrorCount<-ErrorCount+1}
    }
  }
  # i and j should now record the element that we have chosen to change
  # what square is 
  WhichSquare<-1+3*((i-1)%/%3)+(j-1)%/%3
  z<-SwitchSomethingWithThis(z,w,WhichSquare,i,j)
  cat("\nNeed to fish this!")
  return(z)
}
...
# a function to generate a random start point
...
# A function to fill a 3x3 square
...
# the 3x3 squares will be correct by construction
Score<-function(z,w)
{
  Score<-0
  for (i in 1:9){
...
Score2<-function(z,w) #gives more weight to clashes with the pre-determined squares
{
  Score<-0
  for (i in 1:9){
    Score<-Score+(9-length(unique(z[i,1:9])))
  ...
  for (i in 1:9){
    for (j in 1:9){
      # any clashes in that row?
      for (k in 1:9){
         if ((w[i,k]==1)&&(z[i,k]==z[i,j])&&(k!=j)){
           Score<-Score+10
         }     
      }
      # any clashes in that column?
      for (k in 1:9){
        if ((w[k,j]==1)&&(z[k,j]==z[i,j])&&(k!=i)){
          Score<-Score+10
        }     
      }
    }
...
MyZ<-read.table("Sudoku_Simple.txt")
...
SimAnneal(Score2,MyZ2,MyFixedPoints,InitialTemp,FinalTemp,TempDecreaseRate)
